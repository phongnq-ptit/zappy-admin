...
const FilterComic = () => {
  ...
  useEffect(() => {
    Promise.all([
      getAuthorComic().then((response) => {
        setAuthor(response.data.results);
      }),
      getGenreComic().then((response) => {
  ...
  const handleRemoveItemLists = () => {
    ...
    deleteComic(selected)
      .then((response) => {
        SuccessSnackbar('Xóa truyện thành công!');
        onChangeComics([
          ...comics.filter((item) => !selected.includes(item.id))
        ]);
        onChangeListMetadata({
          ...listMetadata,
